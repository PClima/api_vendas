...
import { UsersTokensRepository } from '@/users/domain/repositories/user-tokens.repository'
...
describe('UserTokensTypeormRepository Integration Tests', () => {
  ...
  beforeEach(async () => {
    ...
    userTokensRepository = new UserTokensTypeormRepository(
      typeormEntityManager.getRepository(UserTokens),
      usersRepository,
    )
  })
  ...
  it('should throw error when token not found', async () => {
    ...
    await expect(userTokensRepository.findByToken(token)).rejects.toThrow(
      new NotFoundError(`User Token not found using token ${token}`),
    )
  })
